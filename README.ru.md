...
1.  Скопируйте из этого репозитария папку jb_script в удобное для вас место на вашем компьютере. 
    Можно её переименовать, но далее эту папку здесь обозначаю `jb_script/`.
1.  Создайте короткий удобный alias для команды `bun <path to jb_script/>/main.js` вашей основной оболочки. 
...
1.  На этом этапе уже всё работает, но вашему редактору кода требуются объявления @types/bun 
...
1.  Цепочка из нескольких `case` перед общим рукавом операций представляет собой:
...
1.  В рукаве операций рецепта обязательно наличие внеблокового 
    (не заключенного в фигурные скобки) `break` или `return`
1.  Рукав `default:` может располагаться только после всех `case` инструкции
