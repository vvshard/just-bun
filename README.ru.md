...
3.  Создайте короткий удобный alias для команды `bun <path to jb_script/>main.js` 
    вашей основной оболочки. Здесь этот alias я буду обозначать `jb`.    
...
Флаги:
  ...
  * `-lf` то же, что и `-l`, но для для файла \<path/to/recipe/file>.ts
...
1.  Все выражения `case` должны быть строковыми литералами в `"` или `'` кавычках, без переносов строк. 
...
2.  Цепочка из нескольких `case` перед общим рукавом операций представляет собой:
    ...
    * необязательные псевдонимы рецепта
    ...
    В список такой рецепт выводится одной строкой: в начале - имя рецепта, затем, если есть
    , через ` | ` псевдонимы, и в конце - все комментарии рецепта в одну строку через пробел.   
...
3.  Во всех рукавах операций рецепта, кроме последнего, обязательно наличие внеблокового 
